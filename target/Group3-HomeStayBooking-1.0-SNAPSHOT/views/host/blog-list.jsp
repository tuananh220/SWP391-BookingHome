...
<main class="host-wrap">
    <div class="host-page-head">
        <div><p class="eyebrow">CHIA SẺ HOMESTAY</p><h1>Bài viết của tôi</h1></div>
    ...
    <c:choose>
        <c:when test="${empty blogs}"><section class="empty-state"><h3>Bạn chưa có bài viết</h3></section></c:when>
        <c:otherwise>
                ...
                <c:forEach items="${blogs}" var="blog">
                    <article class="customer-blog-card">
                        <div class="customer-blog-thumb">
                            <c:choose>
                                <c:when test="${not empty blog.thumbnailUrl}"><img src="<c:out value='${blog.thumbnailUrl}'/>" alt="Blog"></c:when>
                                <c:otherwise><span>HOST STORY</span></c:otherwise>
                            </c:choose>
                        </div>
