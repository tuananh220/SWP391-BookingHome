    ...
    <main class="property-form-wrap">
        ...
        <form method="post" action="${pageContext.request.contextPath}/host/homestay-form" class="property-form">
            ...
            <section class="form-section">
                ...
                <label class="full">
                    Tên homestay
                    <input type="text" name="title" maxlength="200" required value="<c:out value='${form.title}'/>">
                </label>
                <label class="full">
                    Mô tả
                    <textarea name="description" rows="6"><c:out value="${form.description}"/></textarea>
                </label>
                <label>
                    Giá mỗi đêm
                    <input type="number" name="pricePerNight" min="0" step="1000" required value="${form.pricePerNight}">
                </label>
                <label>
                    Số khách tối đa
                    <input type="number" name="maxGuests" min="1" required value="${form.maxGuests > 0 ? form.maxGuests : 1}">
                </label>
            </section>
            <section class="form-section">
                <h2>Địa điểm</h2>
                <label class="full">
                    Địa chỉ
                    <input type="text" name="address" maxlength="255" required value="<c:out value='${form.address}'/>">
                </label>
                <label>
                    Thành phố
                    <input type="text" name="city" maxlength="100" required value="<c:out value='${form.city}'/>">
                </label>
                <label>
                    Quận/Huyện
                    <input type="text" name="district" maxlength="100" value="<c:out value='${form.district}'/>">
                </label>
                <label>
                    Vĩ độ
                    <input type="number" name="latitude" min="-90" max="90" step="0.00000001" value="${form.latitude}">
                </label>
                <label>
                    Kinh độ
                    <input type="number" name="longitude" min="-180" max="180" step="0.00000001" value="${form.longitude}">
                </label>
            </section>
            <section class="form-section">
                ...
                <div class="policy-picker-grid">
                    <c:forEach items="${policies}" var="policy">
                        <label class="policy-picker-card">
                            <input type="radio" name="cancellationPolicyId" value="${policy.policyId}" required ${form.cancellationPolicyId == policy.policyId ? 'checked' : ''}>
                            <div class="policy-card-head">
                                <div>
                                    <small>POLICY #${policy.policyId}</small>
                                    <h2><c:out value="${policy.policyName}"/></h2>
                                </div>
                            ...
                            <div class="policy-rules">
                                ...
                                <span>
                                    <b><fmt:formatNumber value="${policy.partialRefundPercent}" maxFractionDigits="0"/> % </b>
                                    trước ${policy.partialRefundDays} ngày
                                </span>
                ...
                <div class="option-grid full">
                    <c:forEach items="${paymentMethods}" var="method">
                        <label class="check-option">
                            ...
                            <span><c:out value="${method.methodName}"/><small>${method.online ? 'Online' : 'Tại homestay'}</small></span>
                        </label>
                    </c:forEach>
                </div>
            </section>
            <section class="form-section">
                ...
                <label class="full">
                    ...
                    <textarea name="imageUrls" rows="7" required placeholder="https://example.com/image-1.jpg&#10;https://example.com/image-2.jpg"><c:out value="${form.imageUrlsText}"/></textarea>
