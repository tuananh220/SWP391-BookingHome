...
<body>
    ...
    <main class="customer-blog-editor-wrap">
        ...
        <form method="post" action="${pageContext.request.contextPath}/customer/blog-form" class="blog-editor customer-editor">
            ...
            <label>
                Tiêu đề
                <input name="title" maxlength="200" required placeholder="Chuyến đi đáng nhớ của tôi..." value="<c:out value='${blog.title}'/>">
            </label>
            <label>
                Slug
                <input name="slug" maxlength="200" placeholder="Để trống để tự tạo" value="<c:out value='${blog.slug}'/>">
            </label>
            <label>
                URL ảnh đại diện
                <input type="url" name="thumbnailUrl" maxlength="500" placeholder="https://..." value="<c:out value='${blog.thumbnailUrl}'/>">
            </label>
            <label>
                Nội dung
                <textarea name="content" rows="20" required placeholder="Hãy kể về hành trình, địa điểm và trải nghiệm của bạn..."><c:out value="${blog.content}"/></textarea>
            </label>
            <div class="customer-blog-note">
                Khi lưu hoặc chỉnh sửa, bài viết sẽ chuyển về trạng thái chờ Admin duyệt.
            </div>
