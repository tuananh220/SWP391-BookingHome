...
<body class="host-body">
    ...
    <main class="voucher-form-wrap">
        ...
        <form method="post" action="${pageContext.request.contextPath}/host/voucher-form" class="voucher-editor">
            ...
            <label>
                Mã voucher
                <input class="code-input" type="text" name="voucherCode" maxlength="50" required placeholder="SUMMER2026" value="<c:out value='${voucher.voucherCode}'/>">
            </label>
            <label>
                Phạm vi áp dụng
            ...
            <div class="voucher-field-grid">
                <label>
                    Phần trăm giảm
                    <input type="number" name="discountRate" min="0.01" max="100" step="0.01" required value="${voucher.discountRate}">
                </label>
                <label>
                    Giảm tối đa
                    <input type="number" name="maxDiscountAmount" min="0" step="1000" placeholder="Để trống nếu không giới hạn" value="${voucher.maxDiscountAmount}">
                </label>
            </div>
            <div class="voucher-field-grid">
                <label>
                    Đơn hàng tối thiểu
                    <input type="number" name="minOrderValue" min="0" step="1000" required value="${voucher.minOrderValue}">
                </label>
                <label>
                    Giới hạn lượt dùng
                    <input type="number" name="usageLimit" min="1" required value="${voucher.usageLimit}">
                </label>
            </div>
            <div class="voucher-field-grid">
                <label>
                    Bắt đầu
                    <input type="datetime-local" name="startDate" required value="${voucher.startDateInput}">
                </label>
                <label>
                    Kết thúc
                    <input type="datetime-local" name="endDate" required value="${voucher.endDateInput}">
                </label>
            </div>
