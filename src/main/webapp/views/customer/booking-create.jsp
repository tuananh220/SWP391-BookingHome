        ...
        <section class="checkout-main">
            ...
            <section class="checkout-section">
                ...
                <form method="get" action="${pageContext.request.contextPath}/booking/create" class="voucher-form">
                    ...
                    <input type="text" name="voucher" maxlength="50" placeholder="Nhập mã voucher" value="<c:out value='${voucherCode}'/>">
                    <button type="submit">Áp dụng</button>
                </form>
                <c:if test="${not empty quote.voucher}">
                    <p class="voucher-ok">
                        Đã áp dụng mã
                        <strong><c:out value="${quote.voucher.voucherCode}"/></strong>
                        – giảm
                        <fmt:formatNumber value="${quote.voucher.discountRate}" maxFractionDigits="0"/>
                        %
                    </p>
                </c:if>
            </section>
                    ...
                    <c:when test="${not empty quote.homestay.cancellationPolicy}">
                        <article class="policy-admin-card">
                            <div class="policy-card-head">
                                <div>
                                    <small>POLICY</small>
                                    <h2><c:out value="${quote.homestay.cancellationPolicy.policyName}"/></h2>
                                </div>
                            ...
                            <div class="policy-rules">
                                ...
                                <span>
                                    <b><fmt:formatNumber value="${quote.homestay.cancellationPolicy.partialRefundPercent}" maxFractionDigits="0"/> % </b>
                                    trước ${quote.homestay.cancellationPolicy.partialRefundDays} ngày
                                </span>
                    ...
                    <div class="payment-options">
                        <c:forEach items="${quote.paymentMethods}" var="method" varStatus="status">
                            <label class="payment-option">
                                <input type="radio" name="paymentMethodId" value="${method.paymentMethodId}" required ${status.first ? 'checked' : ''}>
                                <span>
                                    <strong><c:out value="${method.methodName}"/></strong>
        ...
        <aside class="price-summary">
            ...
            <c:forEach items="${quote.nights}" var="night">
                <div class="price-row">
                    ...
                    <span><fmt:formatNumber value="${night.nightPrice}" pattern="#,##0"/> ₫</span>
                </div>
            </c:forEach>
            <div class="price-row subtotal">
                <span>Tạm tính</span>
                <span><fmt:formatNumber value="${quote.originalAmount}" pattern="#,##0"/> ₫</span>
            </div>
            ...
            <div class="price-total">
                <span>Tổng cộng</span>
                <strong><fmt:formatNumber value="${quote.totalAmount}" pattern="#,##0"/> ₫</strong>
            </div>
        </aside>
    </main>
</body>
