...
<main class="customer-blog-editor-wrap">
    ...
    <form method="post" action="${pageContext.request.contextPath}/host/blog-form" class="blog-editor customer-editor">
        ...
        <label>Tiêu đề<input name="title" maxlength="200" required value="<c:out value='${blog.title}'/>"></label>
        <label>Slug<input name="slug" maxlength="200" placeholder="Để trống để tự tạo" value="<c:out value='${blog.slug}'/>"></label>
        <label>URL ảnh đại diện<input type="url" name="thumbnailUrl" maxlength="500" value="<c:out value='${blog.thumbnailUrl}'/>"></label>
        <label>Nội dung<textarea name="content" rows="16" required><c:out value="${blog.content}"/></textarea></label>
