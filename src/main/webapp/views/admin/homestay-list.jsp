                ...
                <tbody>
                    <c:forEach items="${homestays}" var="item">
                        <tr>
                            ...
                            <td><strong><c:out value="${item.title}"/></strong><small><c:out value="${item.district}"/> , <c:out value="${item.city}"/></small></td>
                            <td><strong><c:out value="${item.hostName}"/></strong><small><c:out value="${item.hostEmail}"/></small></td>
                            <td><fmt:formatNumber value="${item.pricePerNight}" pattern="#,##0"/> ₫</td>
                            <td>${item.maxGuests} khách</td>
                            <td> ★ <fmt:formatNumber value="${item.averageRating}" maxFractionDigits="1"/> (${item.reviewCount}) </td>
                            <td><span class="booking-status status-${item.status}"><c:out value="${item.status}"/></span></td>
                            <td><a href="${pageContext.request.contextPath}/admin/homestay-detail?id=${item.homestayId}">Chi tiết</a></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
