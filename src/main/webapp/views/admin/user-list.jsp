...
<body class="admin-body">
    ...
    <main class="admin-main">
        ...
        <form method="get" action="${pageContext.request.contextPath}/admin/users" class="admin-filter">
            <input type="search" name="keyword" placeholder="Tên, email hoặc số điện thoại" value="<c:out value='${param.keyword}'/>">
            <select name="role">
                <option value="">Tất cả vai trò</option>
                <c:forEach items="${roles}" var="role">
                    <option value="<c:out value='${role.roleName}'/>" ${param.role == role.roleName ? 'selected' : ''}>
            ...
            <table class="admin-table">
                ...
                <tbody>
                    <c:forEach items="${users}" var="user">
                        <tr>
                            <td>#${user.userId}</td>
                            <td><strong><c:out value="${user.fullName}"/></strong><small><c:out value="${user.address}"/></small></td>
                            <td><strong><c:out value="${user.email}"/></strong><small><c:out value="${user.phoneNumber}"/></small></td>
                            <td><span class="role-chip"><c:out value="${user.roleName}"/></span></td>
                            <td><span class="booking-status status-${user.status}"><c:out value="${user.status}"/></span></td>
                            <td>${user.createdAt}</td>
                            <td>
                                <a href="${pageContext.request.contextPath}/admin/user-form?id=${user.userId}">Chỉnh sửa</a>
