...
public class CustomerBookingDetailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CustomerBookingDetailController</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CustomerBookingDetailController at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        Booking booking = service.getCustomerBooking(
        ...
        request.setAttribute("estimatedRefund", service.estimateRefund(booking));
        ReviewService reviewService = new ReviewService();
        request.setAttribute(
                "canReview",
                reviewService.canReview(bookingId, currentUser.getUserId())
        );
        PaymentService paymentService = new PaymentService();
        request.setAttribute(
                "pendingOnlinePayment",
                paymentService.getPendingOnlinePayment(
                        bookingId, currentUser.getUserId()
                )
        );
        request.getRequestDispatcher(
                "/views/customer/booking-detail.jsp"
