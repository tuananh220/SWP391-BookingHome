    ...
    <body class="host-body">
        ...
        <main class="host-wrap">
            <div class="host-page-head">
                <div><p class="eyebrow">TÀI SẢN CỦA TÔI</p><h1>Quản lý homestay</h1></div>
            ...
            <c:choose>
                <c:when test="${empty homestays}"><section class="empty-state"><h3>Bạn chưa có homestay</h3><p>Hãy thêm nơi lưu trú đầu tiên để bắt đầu nhận booking.</p></section></c:when>
                    <c:otherwise>
                        ...
                        <c:forEach items="${homestays}" var="item">
                            <article class="host-property-card">
                                <div class="host-property-image">
                                    <c:choose><c:when test="${not empty item.primaryImageUrl}"><img src="<c:out value='${item.primaryImageUrl}'/>" alt="Homestay"></c:when><c:otherwise><span>HOMESTAY</span></c:otherwise></c:choose>
                                ...
                                <div class="host-property-content">
                                    ...
                                    <div class="host-property-stats"><span><strong><fmt:formatNumber value="${item.pricePerNight}" pattern="#,##0"/> ₫</strong>/đêm</span><span>★ <fmt:formatNumber value="${item.averageRating}" maxFractionDigits="1"/> (${item.reviewCount})</span></div>
                                    <div class="host-property-actions">
                                        ...
                                        <form method="post" action="${pageContext.request.contextPath}/host/homestay-action">
                                            ...
                                            <c:choose>
                                                <c:when test="${item.status == 'Hidden'}"><button type="submit" name="action" value="show">Gửi duyệt lại</button></c:when>
                                                <c:otherwise><button type="submit" name="action" value="hide">Ẩn</button></c:otherwise>
                                            </c:choose>
                                        </form>
                                    </div>
                                </div>
