    ...
    <main class="admin-main">
        ...
        <form class="admin-filter" method="get" action="${pageContext.request.contextPath}/admin/reviews">
            ...
            <label class="reported-check">
                <input type="checkbox" name="reported" value="true" ${param.reported == 'true' ? 'checked' : ''}>
                Có báo cáo chờ xử lý
            </label>
                ...
                <tbody>
                    <c:forEach items="${reviews}" var="review">
                        <tr>
                            ...
                            <td><strong><c:out value="${review.customerName}"/></strong><small>Booking #${review.bookingId}</small></td>
                            <td><c:out value="${review.homestayTitle}"/></td>
                            <td><strong class="admin-stars">${review.ratingStars}/5 ★</strong></td>
                            <td class="review-excerpt"><c:out value="${review.comment}"/></td>
                            <td>
                                <span class="booking-status ${review.visible ? 'status-Active' : 'status-Blocked'}"> ${review.visible ? 'Visible' : 'Hidden'} </span>
                            </td>
                            <td><c:if test="${review.reportCount > 0}"><span class="report-badge">${review.reportCount} pending</span></c:if></td>
                            <td><a href="${pageContext.request.contextPath}/admin/review-detail?id=${review.reviewId}">Chi tiết</a></td>
                        </tr>
                    </c:forEach>
                </tbody>
            </table>
