    ...
    <main class="admin-main">
        <header class="admin-page-head">
            <div>
                ...
                <h1><c:out value="${review.homestayTitle}"/></h1>
                <p> Khách hàng: <strong><c:out value="${review.customerName}"/></strong> · Booking #${review.bookingId} </p>
            </div>
        ...
        <div class="admin-review-grid">
            <div>
                <form method="post" action="${pageContext.request.contextPath}/admin/review-edit" class="admin-editor admin-review-editor">
                    ...
                    <label>
                        Số sao
                        <input type="number" name="ratingStars" min="1" max="5" required value="${review.ratingStars}">
                    </label>
                    <label>
                        Bình luận
                        <textarea name="comment" rows="7"><c:out value="${review.comment}"/></textarea>
                    </label>
                    <label>
                        Phản hồi của chủ nhà
                        <textarea name="hostResponse" rows="5"><c:out value="${review.hostResponse}"/></textarea>
                    </label>
                    <label class="visible-check">
                        <input type="checkbox" name="visible" ${review.visible ? 'checked' : ''}>
                    ...
                    <c:choose>
                        ...
                        <c:otherwise>
                            <c:forEach items="${review.reports}" var="report">
                                <article>
                                    <div>
                                        <strong><c:out value="${report.reporterName}"/></strong>
                                        <span class="booking-status status-${report.status}"><c:out value="${report.status}"/></span>
                                    </div>
                                    <p><c:out value="${report.reason}"/></p>
