        ...
        <main class="checkout-wrap">
            <section class="checkout-main">
                ...
                <article class="trip-property">
                    <div>
                        <p class="eyebrow"><c:out value="${quote.homestay.city}"/></p>
                        <h2><c:out value="${quote.homestay.title}"/></h2>
                        <p><c:out value="${quote.homestay.address}"/></p>
                    </div>
                ...
                <section class="checkout-section">
                    ...
                    <div class="checkout-data">
                        <div><span>Nhận phòng</span><strong>${quote.checkInDate}</strong></div>
                        <div><span>Trả phòng</span><strong>${quote.checkOutDate}</strong></div>
                        <div><span>Số đêm</span><strong>${quote.totalNights}</strong></div>
                        <div><span>Số khách</span><strong>${quote.totalGuests}</strong></div>
                    </div>
                </section>
                ...
                <section class="checkout-section">
                    ...
                    <c:if test="${not empty quote.voucher}">
                        <p class="voucher-ok">Đã áp dụng mã <strong><c:out value="${quote.voucher.voucherCode}"/></strong> – giảm <fmt:formatNumber value="${quote.voucher.discountRate}" maxFractionDigits="0"/>%</p>
                    </c:if>
                </section>
                        ...
                        <c:when test="${not empty quote.homestay.cancellationPolicy}">
                            <article class="policy-admin-card">
                                <div class="policy-card-head">
                                    <div><small>POLICY</small><h2><c:out value="${quote.homestay.cancellationPolicy.policyName}"/></h2></div>
                                    <span class="booking-status status-Active">ACTIVE</span>
                                </div>
                                <p><c:out value="${quote.homestay.cancellationPolicy.description}"/></p>
                                <div class="policy-rules">
                                    <span><b>100%</b> trước ${quote.homestay.cancellationPolicy.fullRefundDays} ngày</span>
                                    <span><b><fmt:formatNumber value="${quote.homestay.cancellationPolicy.partialRefundPercent}" maxFractionDigits="0"/>%</b> trước ${quote.homestay.cancellationPolicy.partialRefundDays} ngày</span>
                ...
                <form method="post" action="${pageContext.request.contextPath}/booking/create" class="confirm-form">
                        ...
                        <div class="payment-options">
                            <c:forEach items="${quote.paymentMethods}" var="method" varStatus="status">
                                <label class="payment-option">
                                    ...
                                    <span><strong><c:out value="${method.methodName}"/></strong><small>${method.online ? 'Thanh toán trực tuyến' : 'Thanh toán tại homestay'}</small></span>
                                </label>
                            </c:forEach>
                    ...
                    <section class="checkout-section">
                        ...
                        <textarea name="note" maxlength="255" rows="4" placeholder="Ví dụ: Tôi dự kiến nhận phòng lúc 14:00"></textarea>
                    </section>
            ...
            <aside class="price-summary">
                ...
                <c:forEach items="${quote.nights}" var="night">
                    <div class="price-row"><span>${night.stayDate}</span><span><fmt:formatNumber value="${night.nightPrice}" pattern="#,##0"/> ₫</span></div>
                        </c:forEach>
                <div class="price-row subtotal"><span>Tạm tính</span><span><fmt:formatNumber value="${quote.originalAmount}" pattern="#,##0"/> ₫</span></div>
                <c:if test="${quote.discountAmount > 0}">
                    <div class="price-row discount"><span>Giảm giá</span><span>− <fmt:formatNumber value="${quote.discountAmount}" pattern="#,##0"/> ₫</span></div>
                </c:if>
                <div class="price-total"><span>Tổng cộng</span><strong><fmt:formatNumber value="${quote.totalAmount}" pattern="#,##0"/> ₫</strong></div>
            </aside>
        </main>
    </body>
</html>
