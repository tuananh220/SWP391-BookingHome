...
<body>
    ...
    <article class="public-blog-detail">
        <header>
            ...
            <p> Bởi <strong><c:out value="${blog.authorName}"/></strong> · <c:out value="${blog.createdAtText}"/></p>
        </header>
        <c:if test="${not empty blog.thumbnailUrl}">
            <img class="blog-cover" src="<c:out value='${blog.thumbnailUrl}'/>" alt="<c:out value='${blog.title}'/>">
        </c:if>
