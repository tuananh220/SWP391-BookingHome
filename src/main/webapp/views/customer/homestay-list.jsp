...
<body>
                    ...
                    <c:when test="${not empty banner.targetUrl}">
                        ...
                        <a href="<c:out value='${bannerLink}'/>">
                            ...
                            <c:if test="${not empty banner.title}">
                                <span><c:out value="${banner.title}"/></span>
                            </c:if>
                        </a>
                    </c:when>
                    <c:otherwise>
    ...
    <section class="search-hero">
        ...
        <form method="get" action="${pageContext.request.contextPath}/homestays" class="search-form">
            <label class="wide">
                Từ khóa
                <input type="text" name="keyword" placeholder="Tên hoặc địa chỉ homestay" value="<c:out value='${param.keyword}'/>">
            </label>
            <label>
                Thành phố
                <select name="city">
                    ...
                    <c:forEach items="${cities}" var="city">
                        <option value="<c:out value='${city}'/>" ${param.city == city ? 'selected' : ''}><c:out value="${city}"/></option>
                    </c:forEach>
                </select>
            </label>
            <label>
            ...
            <label>
                Giá từ
                <input type="number" name="minPrice" min="0" step="1000" value="<c:out value='${param.minPrice}'/>">
            </label>
            <label>
                Giá đến
                <input type="number" name="maxPrice" min="0" step="1000" value="<c:out value='${param.maxPrice}'/>">
            </label>
            <label>
                Tiện ích
                <select name="amenityId">
                    ...
                    <c:forEach items="${amenities}" var="amenity">
                        <option value="${amenity.amenityId}" ${param.amenityId == amenity.amenityId ? 'selected' : ''}>
