                        ...
                        <article class="host-change-card">
                            <div class="change-request-head">
                                <div>
                                    ...
                                    <h2><c:out value="${item.homestayTitle}"/></h2>
                                    <p> Khách: <strong><c:out value="${item.customerName}"/></strong></p>
                                </div>
                                <span class="booking-status status-${item.status}"><c:out value="${item.status}"/></span>
                            </div>
                            <div class="change-request-data">
                                <span><b>Loại</b> ${item.requestType == 'Extension' ? 'Gia hạn' : 'Trả sớm'} </span>
                                <span><b>Ngày cũ</b> ${item.originalCheckOutDate} </span>
                                <span><b>Ngày mới</b> ${item.requestedCheckOutDate} </span>
                                <span>
                                    <b>${item.requestType == 'Extension' ? 'Thu thêm' : 'Hoàn dự kiến'}</b>
                                    <fmt:formatNumber value="${item.requestType == 'Extension' ? item.extraAmount : item.refundAmount}" pattern="#,##0"/>
                                    ₫
                                </span>
                            </div>
                            <c:if test="${not empty item.customerNote}">
                                <p><strong>Ghi chú:</strong><c:out value="${item.customerNote}"/></p>
                            </c:if>
