                ...
                <section class="host-panel">
                    ...
                    <div class="host-info-grid">
                        ...
                        <div>
                            ...
                            <strong><c:out value="${empty booking.customerPhone ? 'Chưa cập nhật' : booking.customerPhone}"/></strong>
                        </div>
                    </div>
                </section>
                <section class="host-panel">
                    ...
                    <c:if test="${not empty booking.note}">
                        <p class="booking-note"><strong>Ghi chú:</strong><c:out value="${booking.note}"/></p>
                    </c:if>
                </section>
                <c:if test="${not empty booking.rejectReason}">
                    <section class="host-panel rejected-note">
                        <h2>Lý do từ chối</h2>
                        <p><c:out value="${booking.rejectReason}"/></p>
                    </section>
                </c:if>
            </div>
            <aside>
                ...
                <c:if test="${booking.bookingStatus == 'Pending' && booking.paymentOnline}">
                    <div class="notice">
                        Booking đang chờ khách hoàn tất thanh toán online và sẽ tự động xác nhận.
                    </div>
                </c:if>
                <c:if test="${canProcess}">
                    <section class="host-panel action-panel">
                        ...
                        <form method="post" action="${pageContext.request.contextPath}/host/booking-action" class="reject-form">
                            ...
                            <label>
                                Lý do từ chối
                                <textarea name="reason" maxlength="255" rows="3" required></textarea>
                            </label>
                    ...
                    <section class="host-panel action-panel">
                        ...
                        <c:if test="${not empty booking.cancellationPolicyName}">
                            <p>
                                <small>
                                    Chính sách:
                                    <c:out value="${booking.cancellationPolicyName}"/>
                                    · hoàn 100% trước ${booking.fullRefundDaysSnapshot} ngày, hoàn
                                    <fmt:formatNumber value="${booking.partialRefundPercentSnapshot}" maxFractionDigits="0"/>
                                    % trước ${booking.partialRefundDaysSnapshot} ngày.
                                </small>
                            </p>
                        </c:if>
                        <form method="post" action="${pageContext.request.contextPath}/host/booking-action" class="reject-form">
