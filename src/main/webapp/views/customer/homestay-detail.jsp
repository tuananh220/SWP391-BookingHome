<%-- Document : homestay-detail Created on : Aug 18, 2026, 4:31:33 PM Author : Admin --%>
        ...
        <%@ taglib prefix="c" uri="jakarta.tags.core" %>
            <%@ taglib prefix="fmt" uri="jakarta.tags.fmt" %>
                ...
                <head>
                    ...
                    <title>
                        <c:out value="${homestay.title}" /> | Homestay
                    </title>
                    ...
                    <main class="detail-wrap">
                        <section class="detail-heading">
                            ...
                            <div class="detail-meta">
                                <span>★
                                    <fmt:formatNumber value="${homestay.averageRating}" maxFractionDigits="1" />
                            ...
                            <c:if
                                ...
                                <form class="favorite-detail-form" method="post"
                                    action="${pageContext.request.contextPath}/customer/favorite-action">
                                    <input type="hidden" name="homestayId" value="${homestay.homestayId}">
                                    <input type="hidden" name="returnPage" value="detail">
                                    <button type="submit" name="action"
                                        value="${isFavorite ? 'remove' : 'add'}">${isFavorite ? '♥ Đã yêu thích' : '♡
                                ...
                                <c:otherwise>
                                    <c:forEach items="${homestay.images}" var="image" begin="0" end="4">
                                        <img src="<c:out value='${image.imageUrl}'/>"
                                            alt="<c:out value='${homestay.title}'/>">
                                    </c:forEach>
                                </c:otherwise>
                            </c:choose>
                        </section>
                        ...
                        <div class="detail-layout">
                                ...
                                <c:if test="${not empty homestay.cancellationPolicy}">
                                    <section class="content-section policy-box">
                                        ...
                                        <div class="policy-rules">
                                            <span><b>100%</b> trước ${homestay.cancellationPolicy.fullRefundDays}
                                                ngày</span>
                                            <span><b>
                                                    <fmt:formatNumber
                                                        value="${homestay.cancellationPolicy.partialRefundPercent}"
                                                        maxFractionDigits="0" />%
                                                </b> trước ${homestay.cancellationPolicy.partialRefundDays} ngày</span>
                                                        ...
                                                        <c:if test="${not empty review.imageUrls}">
                                                            <div class="review-images">
                                                                <c:forEach items="${review.imageUrls}" var="imageUrl">
                                                                    <img src="${pageContext.request.contextPath}/<c:out value='${imageUrl}'/>"
                                                                        alt="Ảnh đánh giá">
                                                                </c:forEach>
                                                            </div>
                                                        </c:if>
                                                        <c:if test="${not empty review.hostResponse}">
                                                            <div class="host-response"><strong>Phản hồi của chủ
                                                                    nhà:</strong>
                            ...
                            <aside class="booking-card">
                                <p class="price"><strong>
                                        <fmt:formatNumber value="${homestay.pricePerNight}" pattern="#,##0" /> ₫
                                    </strong> / đêm</p>
                                <form id="bookingForm" method="get"
                                    ...
                                    <div class="date-grid">
                                        <label>Nhận phòng
                                            <input type="date" id="detailCheckIn" name="checkIn" required
                                                value="<c:out value='${checkIn}'/>">
                                        </label>
                                        <label>Trả phòng
                                            <input type="date" id="detailCheckOut" name="checkOut" required
                                                value="<c:out value='${checkOut}'/>">
                                        </label>
                                    </div>
                                    <label>Số khách
                                        <input type="number" name="guests" min="1" max="${homestay.maxGuests}" required
                                            value="${not empty guests ? guests : ''}" placeholder="Nhập số khách">
                                    </label>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <a class="book-login" href="${pageContext.request.contextPath}/login">Đăng
                                                nhập để đặt phòng</a>
                                        </c:otherwise>
                                    </c:choose>
                                </form>
                ...
                <script>
                    // ── Date validation (homestay-detail booking card) ──────────────────
                    (function () {
                        var checkInEl = document.getElementById('detailCheckIn');
                        var checkOutEl = document.getElementById('detailCheckOut');
                        if (!checkInEl || !checkOutEl) return;

                        // Tính ngày hôm nay theo local time
                        var today = new Date();
                        ...
                        // Gán min = hôm nay → trình duyệt khoá ngày quá khứ trong date-picker
                        checkInEl.min = todayStr;
                        checkOutEl.min = todayStr;

                        // Khi checkIn thay đổi → cập nhật min của checkOut
                        checkInEl.addEventListener('change', function () {
                            var ciVal = checkInEl.value;
                            if (ciVal) {
                                var ciDate = new Date(ciVal);
                                ciDate.setDate(ciDate.getDate() + 1);
                                var coMin = ciDate.toISOString().slice(0, 10);
                                checkOutEl.min = coMin;
                                // Nếu checkOut hiện tại <= checkIn mới → reset
                        ...
                        // Khi checkOut thay đổi → cập nhật max của checkIn
                        checkOutEl.addEventListener('change', function () {
                            ...
                            if (coVal) {
                                ...
                                checkInEl.max = coDate.toISOString().slice(0, 10);
                            } else {
                        ...
                        // Kích hoạt ngay lúc load nếu đã có giá trị từ search
                        if (checkInEl.value) {
