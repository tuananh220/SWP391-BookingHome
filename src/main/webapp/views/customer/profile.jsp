...
<body class="dashboard-body">
    ...
    <main class="profile-wrap">
        <form method="post" action="${pageContext.request.contextPath}/profile" class="auth-card profile-card">
            ...
            <label>
                Họ và tên
                <input type="text" name="fullName" maxlength="100" required value="<c:out value='${sessionScope.currentUser.fullName}'/>">
            </label>
            <label>
                Số điện thoại
                <input type="tel" name="phoneNumber" maxlength="20" value="<c:out value='${sessionScope.currentUser.phoneNumber}'/>">
            </label>
            <label>
                Địa chỉ
                <input type="text" name="address" maxlength="255" value="<c:out value='${sessionScope.currentUser.address}'/>">
            </label>
            <label>
                URL ảnh đại diện
                <input type="url" name="avatarUrl" maxlength="500" value="<c:out value='${sessionScope.currentUser.avatarUrl}'/>">
            </label>
        ...
        <form method="post" action="${pageContext.request.contextPath}/change-password" class="auth-card profile-card">
            ...
            <c:if test="${not empty requestScope.passwordSuccess}">
                <div class="alert success">
                    <c:out value="${requestScope.passwordSuccess}"/>
                </div>
            </c:if>
