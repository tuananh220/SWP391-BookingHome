            ...
            <div class="booking-detail-grid">
                <div>
                    ...
                    <section class="detail-panel">
                        ...
                        <div class="detail-data-grid">
                            <div>
                                ...
                                <strong>
                                    <c:out value="${empty booking.paymentMethodName ? 'Không phát sinh' : booking.paymentMethodName}"/>
                                </strong>
                            </div>
                            <div>
                    ...
                    <c:if test="${booking.bookingStatus == 'Cancelled'}">
                        <section class="detail-panel cancelled-panel">
                            <h2>Thông tin hủy</h2>
                            <p>
                                <strong>Người hủy:</strong> <c:out value="${booking.cancelledBy}"/>
                            </p>
                            <p>
                                <strong>Lý do:</strong> <c:out value="${booking.cancelReason}"/>
                            </p>
                            <p>
                                <strong>Tiền hoàn:</strong> <fmt:formatNumber value="${booking.refundAmount}" pattern="#,##0"/> ₫</p>
                        </section>
                    </c:if>
                    ...
                    <c:if test="${booking.bookingStatus == 'Confirmed'}">
                        <section class="detail-panel stay-change-invite">
                            <div>
                                <p class="eyebrow">THỜI GIAN LƯU TRÚ</p>
                                <h2>Bạn muốn thay đổi ngày trả phòng?</h2>
                                <p>Gửi yêu cầu gia hạn hoặc trả phòng sớm đến chủ nhà.</p>
                            </div>
                            <div class="detail-actions">
                                <a href="${pageContext.request.contextPath}/customer/stay-change-form?bookingId=${booking.bookingId}">Tạo yêu cầu</a>
                                <a href="${pageContext.request.contextPath}/customer/stay-change-requests">Xem các yêu cầu của tôi</a>
                            </div>
                        </section>
                    </c:if>
                    ...
                    <c:if test="${booking.bookingStatus == 'Pending' || booking.bookingStatus == 'Confirmed'}">
                        <section class="detail-panel cancel-panel">
                            <h2>Hủy booking</h2>
                            <p>Số tiền hoàn dự kiến theo chính sách: <strong>
                                    <fmt:formatNumber value="${estimatedRefund}" pattern="#,##0"/> ₫</strong>
                            </p>
                            <form method="post" action="${pageContext.request.contextPath}/customer/cancel-booking" class="cancel-form">
                                <input type="hidden" name="bookingId" value="${booking.bookingId}">
                                <label>Lý do hủy
                                    <textarea name="reason" rows="3" maxlength="255" required placeholder="Cho chúng tôi biết lý do bạn muốn hủy">
                                    </textarea>
                                </label>
                ...
                <aside class="detail-price-panel">
                    ...
                    <div>
                        ...
                        <span>
                            <fmt:formatNumber value="${booking.originalAmount}" pattern="#,##0"/> ₫</span>
                    </div>
                    <div class="discount">
                        <span>Giảm giá</span>
                        <span>− <fmt:formatNumber value="${booking.discountAmount}" pattern="#,##0"/> ₫</span>
                    </div>
